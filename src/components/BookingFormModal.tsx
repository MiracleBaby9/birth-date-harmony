...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { loadRazorpayScript, openRazorpayCheckout } from "@/lib/razorpay";
import { CREATE_ORDER_URL, VERIFY_PAYMENT_URL, SAVE_FORM_PROGRESS_URL } from "@/lib/payment-config";
...
const BookingFormModal = ({ open, onOpenChange, packageName, packagePrice, addons }: BookingFormModalProps) => {
  ...
  // Generated once per modal session and reused for BOTH the early
  // auto-save (below) and the final create-order call — so the real order
  // UPDATEs this same row instead of creating a duplicate one.
  ...
  // Fresh order id each time the modal is opened for a new booking —
  // otherwise, since this component stays mounted between opens, an old
  // session's id could get reused for an unrelated later booking.
  useEffect(() => {
    if (open) orderIdRef.current = null;
  }, [open]);
  ...
  // never blocks the form or the eventual real booking.
  const autoSaveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    if (!open || isPaying || paymentStatus !== "idle") return;

    const hasEmail = !!form.email && form.email.includes("@") && form.email.includes(".");
    const hasWhatsapp = !!form.whatsapp && form.whatsapp.replace(/\D/g, "").length >= 10;
    if (!hasEmail && !hasWhatsapp) return;

    if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current);
    autoSaveTimerRef.current = setTimeout(() => {
      fetch(SAVE_FORM_PROGRESS_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          customerName: form.motherName || form.fatherName || "",
  ...
  }, [open, form, isPaying, paymentStatus, packageName]);
  const toggleAddon = (id: string) =>
    setSelectedAddonIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
    ...
    try {
      ...
      const orderRes = await fetch(CREATE_ORDER_URL, {
        ...
        body: JSON.stringify({
          ...
          // Send customer details + form snapshot NOW, before Razorpay even
          // opens — not just on successful payment. Without this, a
          // customer who fills the form and then closes the Razorpay
          // window (or whose payment fails) leaves literally zero trace:
          // previously only { amount, receipt } reached the backend here,
          // so there was no name/email/phone to follow up an abandoned
          // booking with. This mirrors what ankshaastra.com's own
          // Payment.tsx already does at the same point in its flow.
          serviceTitle: packageName,
          sourceWebsite: "miraclebaby.ankshaastra.com",
          orderType: "service",
          customerName: form.motherName || form.fatherName || "",
