...
// NOTE: kept for manual/admin use (e.g. a future "resend confirmation"
// action) but no longer called automatically from sendBookingNotifications()
// below — see the FIX comment there for why.
export async function sendBookingEmailNotification({ booking, packageName, amountInPaise, orderId, paymentId }) {
  const customerEmail = booking.email;
  const adminEmail = process.env.ADMIN_EMAIL || contactEmail;
...
// FIX: this used to also call sendBookingEmailNotification() above, which
// sent a *second*, independently-formatted email to both the customer and
// the admin (admin copy went to "social@ankshaastra.com" — the "social"
// sender the client flagged). That email is redundant now that Miracle
// Baby is connected to the Ankshaastra hub: the hub's own automation
// already sends the official "no-reply" confirmation email with the PDF
// invoice attached after syncOrderToHubCrm() runs. Email-sending is
// removed here; WhatsApp notifications (customer + admin) are unchanged.
export async function sendBookingNotifications(payload) {
  const result = {
    email: { success: false, skipped: true, reason: "disabled: hub CRM email replaces this" },
    whatsapp: { success: false, skipped: true },
  };
